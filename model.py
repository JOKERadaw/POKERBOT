...
class PositionalEncoding(nn.Module):
    def __init__(self,d_model:int,seq_len:int,dropout:float)->None:
        ...
        pe=pe.unsqueeze(0)# vaffanculo (1,Seq_len,d_model)
        self.register_buffer("pe",pe)#for saving whatever the fuck i am doing
    def forward(self,x):
...
class MultiHeadAttentionBlock(nn.Module):
    ...
    def forward(self,q,k,v,mask):
        #QUI è il PUNTO IN CUI MI SONO DETTO VAFFANCULOOO VOGLIO CREARE UNA COSA MIA
        #E COSTRUIRò uN CENSURATORE QUINDI APPLICHERO MODIFICHE PERSONALI PER FARLO 
        #FUNZIONARE tipo la mask non la uso, però la metto per chiarezza
        query=self.w_q(q)
        key=self.w_k(k)
        value=self.w_v(v)
    ...
    def attention(query,key,value,mask,dropout:nn.Dropout):
        ...
        if mask is not None:
            ...
            #mask will be none! yikes
            #la cocaina in bustina sta chiccandoo
...
class ResidualConnectdion(nn.Module):
...
# ogni battito di tastiera è mio sudore +sangue, amen
class EncoderBlock(nn.Module):
    def __init__(self,self_attention_block:MultiHeadAttentionBlock,feed_forward_block:FeedForwardBlock,dropout:float)-> None:
        super().__init__()
        self.self_attention_block=self_attention_block
        self.feed_forward_block=feed_forward_block
        self.residual_connections=nn.ModuleList([ResidualConnectdion(dropout) for _ in range(2)])
...
def build_transformer(src_vocab_size:int,tgt_vocab_size:int,src_seq_len:int,tgt_seq_len:int,d_model:int=512,N:int=6,h:int=8,dropout:float=0.1,d_ff=2048)->Transessuale:
